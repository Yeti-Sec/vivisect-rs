...
fn load_vectors(name: &str) -> Option<VectorFile> {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(name);
    if !path.exists() {
        eprintln!("Vector file not found: {}", path.display());
        return None;
    }
...
fn parity_spec1_pe32_import_addresses() {
    ...
    let python_addrs: HashSet<u64> = vectors.spec1_imports.iter()
        .map(|i| i.iat_address)
        .collect();
    let rust_addrs: HashSet<u64> = rust_imports.iter()
        .map(|i| i.address)
        .collect();
...
fn parity_spec3_pe32_disasm_full() {
    ...
    eprintln!("  Python instructions: {}", vectors.spec3_disasm.total_instructions);
    eprintln!("  Size match:          {}/{} ({:.1}%)", size_match, total, pct(size_match, total));
    eprintln!("  Mnemonic exact:      {}/{} ({:.1}%)", mnem_match, total, pct(mnem_match, total));
    eprintln!("  Mnemonic synonym:    {} (semantically equivalent)", mnem_synonym);
    eprintln!("  Mnemonic effective:  {}/{} ({:.1}%)", mnem_match + mnem_synonym, total, pct(mnem_match + mnem_synonym, total));
    eprintln!("  Call flag:           {}/{} ({:.1}%)", call_match, total, pct(call_match, total));
    eprintln!("  Return flag:         {}/{} ({:.1}%)", ret_match, total, pct(ret_match, total));
    eprintln!("  Falls-through flag:  {}/{} ({:.1}%)", fall_match, total, pct(fall_match, total));

    assert!(total > 1000, "Expected 1000+ instructions, got {}", total);
    assert!(pct(size_match, total) > 99.0, "Size parity below 99%");
    assert!(pct(mnem_match + mnem_synonym, total) > 99.0, "Mnemonic parity (with synonyms) below 99%");
    assert!(pct(call_match, total) > 99.0, "Call flag parity below 99%");
    assert!(pct(ret_match, total) > 99.0, "Return flag parity below 99%");
    assert!(pct(fall_match, total) > 95.0, "Falls-through parity below 95%");
}
...
fn parity_spec4_pe32_function_discovery() {
    ...
    let python_funcs: HashSet<u64> = vectors.spec4_codeflow.functions.iter()
        .map(|f| f.function_va)
        .collect();
...
fn parity_spec4_pe32_block_boundaries() {
    ...
    let python_funcs: HashMap<u64, &VecCodeflowFunc> = vectors.spec4_codeflow
        .functions.iter()
    ...
    for fva in rust_funcs.iter() {
        ...
        let py_block_starts: HashSet<u64> = py_func.blocks.iter()
            .map(|b| b.start)
            .collect();

        let rust_blocks = ws.get_function_blocks(*fva);
        let rust_block_starts: HashSet<u64> = rust_blocks.iter()
            .map(|b| b.va)
            .collect();
    ...
    eprintln!("  Blocks matched:         {} ({:.1}%)", blocks_matched, block_parity);
    eprintln!("  Parity excl. zero-cov:  {:.1}%",
    ...
    let excl_zero_cov_parity = pct(blocks_matched, blocks_total.saturating_sub(zero_coverage_py_blocks));
    if funcs_compared > 0 {
        assert!(
...
fn parity_spec7_pe32_cfg_edges() {
    ...
    let python_cfg: HashMap<u64, HashMap<u64, HashSet<u64>>> = vectors.spec7_cfg
        .functions.iter()
        .map(|f| {
            let edges: HashMap<u64, HashSet<u64>> = f.edges.iter()
    ...
    for fva in rust_funcs.iter() {
        ...
        for block in &rust_blocks {
            if let Some(py_succs) = py_edges.get(&block.va) {
                let rust_succs: HashSet<u64> = ws.get_block_successors(block.va)
                    .into_iter()
                    .collect();
    ...
    eprintln!("  Edges matched:      {} ({:.1}%)", edges_matched, edge_parity);
...
fn parity_spec8_pe32_xref_count() {
    ...
    let python_xref_set: HashSet<(u64, u64)> = vectors.spec8_xrefs.xrefs.iter()
...
fn parity_summary_pe32() {
    ...
    eprintln!("  Instructions: {}", vectors.spec3_disasm.total_instructions);
...
fn parity_spec2_elf32_entry_point() {
    ...
    eprintln!("  Python entries: {:?}", py_entries.iter().map(|e| format!("{:#x}", e)).collect::<Vec<_>>());
...
fn parity_spec2_elf32_sections() {
    ...
    let py_segment_ranges: Vec<(u64, u64)> = vectors.segments.iter()
        .map(|s| (s.va, s.va + s.size as u64))
        .collect();

    let rust_executable_sections: Vec<_> = rust_sections.iter()
    ...
    for s in &rust_executable_sections {
        eprintln!("    {:#010x} {} (size {})", s.virtual_address, s.name, s.virtual_size);
    }
    ...
    assert!(has_overlap, "No Rust section overlaps with Python segment ranges");
}
...
fn goelf_sample_path() -> std::path::PathBuf {
    Path::new(r"C:\Users\yeti-sec\Desktop\floss_test_samples")
        .join("b02337d82c44ed46e5b186bd54cde717be39da81a29fb332090d10a5c444ccb6")
}
...
fn parity_spec2_elf64_format_detection() {
    ...
    assert!(
        ...
        "Architecture mismatch: Python={}, Rust={}",
        vectors.metadata.architecture, rust_arch
    );
}
...
fn parity_spec2_elf64_sections() {
    ...
    let rust_text: Vec<_> = rust_sections.iter()
        .filter(|s| s.executable && s.virtual_address > 0)
        .collect();
...
fn parity_spec2_elf64_function_discovery() {
    ...
    let ws = match load_workspace("b02337d82c44ed46e5b186bd54cde717be39da81a29fb332090d10a5c444ccb6") {
        Some(w) => w,
        None => return,
    };

    let py_funcs: HashSet<u64> = vectors.spec4_codeflow.functions.iter()
        .map(|f| f.function_va)
        .collect();
    let rust_funcs: HashSet<u64> = ws.get_functions().into_iter().collect();
    ...
    assert!(
        rust_funcs.len() > 0,
        "No functions discovered in ELF64"
    );
}
...
fn parity_summary_elf64() {
    ...
    eprintln!("\n============================================================");
    eprintln!("  PARITY VECTOR SUMMARY: {} (ELF64 Go x86-64)", vectors.file);
    eprintln!("============================================================");
    eprintln!("  Source:       {}", vectors.source);
    eprintln!("  Architecture: {}", vectors.metadata.architecture);
    eprintln!("  Platform:     {}", vectors.metadata.platform);
