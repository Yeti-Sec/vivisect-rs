...
fn test_workspace_locations() {
    ...
    workspace.add_location(0x402000, 14, LocationType::String, Some("Hello, World!".to_string()));
...
fn test_x86_32_disassembly() {
    ...
    // mov eax, 0x12345678
    let op = disasm.disassemble(&[0xB8, 0x78, 0x56, 0x34, 0x12], 0x401000).unwrap();
...
fn test_x86_64_disassembly() {
    ...
    let op = disasm.disassemble(&[0x48, 0x83, 0xEC, 0x20], 0x401000).unwrap();
...
fn test_x86_call_instruction() {
    ...
    // call rel32
    let op = disasm.disassemble(&[0xE8, 0xFB, 0xFF, 0xFF, 0xFF], 0x401000).unwrap();
...
fn test_string_detection() {
    use vivisect::analysis::strings::{detect_ascii_string, detect_utf16le_string, find_strings, StringAnalysisConfig};
...
fn test_string_classification() {
    use vivisect::analysis::strings::{is_path_string, is_url_string, is_registry_string};
...
#[test]
fn test_utils() {
    use vivisect::utils::{align_up, align_down, is_aligned, sign_extend, extract_bits};
...
fn test_workspace_roundtrip() {
    ...
    vivisect::storage::save_workspace(&workspace, &path, vivisect::storage::StorageFormat::Json).unwrap();
