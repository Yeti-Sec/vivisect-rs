...
use vivisect::envi::archs::x86::X86Disassembler;
...
/// Load the Python-generated symboliks vectors, or `None` (with an explicit
/// named skip) when the fixture is absent. Generating it requires an installed
/// Python `vivisect` (`fixtures/generate_symboliks_vectors.py`), a documented
/// external blocker — see PARITY.md.
fn load_symboliks_vectors() -> Option<SymboliksVectorFile> {
    let path = common::generated_fixture("vectors_symboliks.json")?;
    let data = std::fs::read_to_string(&path).expect("Failed to read vectors_symboliks.json");
    Some(serde_json::from_str(&data).expect("Failed to parse vectors_symboliks.json"))
}
...
fn build_reduction_expr(test: &ReductionTest) -> Option<SymbolicValue> {
    ...
    let _rbx = || SymbolicValue::var("rbx", 8);
    ...
    let expr = match (test.category.as_str(), test.description.as_str()) {
        ...
        ("const_fold", "overflow wraps 8-bit") => SymbolicValue::oper(Op::Add, c(0xFF, 1), c(1, 1)),
        ...
        ("identity", "x & 0xFFFFFFFF") => SymbolicValue::oper(Op::And, eax(), c(0xFFFFFFFF, 4)),
        ...
        // Constraint folding
        ("constraint", "x == x") => SymbolicValue::constraint(ConstraintOp::Eq, eax(), eax()),
        ("constraint", "x != x") => SymbolicValue::constraint(ConstraintOp::Ne, eax(), eax()),
        ("constraint", "x < x") => SymbolicValue::constraint(ConstraintOp::Lt, eax(), eax()),
        ("constraint", "x <= x") => SymbolicValue::constraint(ConstraintOp::Le, eax(), eax()),
        ("constraint", "x > x") => SymbolicValue::constraint(ConstraintOp::Gt, eax(), eax()),
        ("constraint", "x >= x") => SymbolicValue::constraint(ConstraintOp::Ge, eax(), eax()),
        ("constraint", "5 == 5") => SymbolicValue::constraint(ConstraintOp::Eq, c(5, 4), c(5, 4)),
        ("constraint", "5 != 5") => SymbolicValue::constraint(ConstraintOp::Ne, c(5, 4), c(5, 4)),
        ("constraint", "3 < 5") => SymbolicValue::constraint(ConstraintOp::Lt, c(3, 4), c(5, 4)),
        ("constraint", "5 < 3") => SymbolicValue::constraint(ConstraintOp::Lt, c(5, 4), c(3, 4)),
...
fn spec5_reduction_constant_folding() {
    let Some(vectors) = load_symboliks_vectors() else {
        return;
    };
    let mut passed = 0;
    let mut failed = 0;
...
fn spec5_reduction_identity_and_annihilation() {
    ...
    assert!(failed == 0, "{} identity/annihilation tests failed", failed);
}
        ...
        if let Some(expected_val) = test.solved_value {
            if reduced.is_discrete() {
                ...
                if test.description.contains("x") {
                    ...
                    eprintln!("  FAIL [constraint] {}: not discrete", test.description);
                    failed += 1;
                }
            }
        } else {
...
fn spec6_i386_translation_effect_counts() {
    ...
    for instr in &vectors.spec6_translation.i386.instructions {
        ...
        let op = match disasm.disassemble(&opcode_bytes, 0x401000) {
            Ok(op) => op,
            Err(e) => {
                diverged.push(format!("{}: Rust disasm failed: {}", instr.description, e));
                total += 1;
                continue;
            }
        };
        ...
        if rust_has_primary {
            ...
            if !py_primary.is_empty() && !reg_match {
                ...
                if py_writes.is_empty() {
                    diverged.push(format!("{}: target register mismatch", instr.description));
                } else {
                    matched += 1;
                }
            } else {
            ...
            diverged.push(format!("{}: no primary effect produced", instr.description));
        }
    }
...
fn spec6_summary() {
    ...
    for (arch_name, arch_data, disasm, ptr_size, va) in [
        ...
        for instr in &arch_data.instructions {
            ...
            let py_count = instr.effect_count.unwrap_or(0);
            trans_total += 1;
            if !effects.is_empty() || py_count == 0 {
                trans_match += 1;
            }
        }
        eprintln!(
