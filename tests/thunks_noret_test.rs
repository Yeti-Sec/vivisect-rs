...
fn test_thunks_found_pe32() {
    ...
    let thunks: Vec<u64> = ws
        .get_functions()
        .into_iter()
        .filter(|&va| ws.is_function_thunk(va))
        .collect();
    let noret: Vec<u64> = ws
        .get_functions()
        .into_iter()
    ...
    assert!(
        !api_annotated.is_empty(),
        "Expected at least some API-annotated functions"
    );
}
