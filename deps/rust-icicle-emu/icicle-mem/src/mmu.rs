...
impl Mmu {
    ...
    pub fn add_write_hook(
        ...
        tracing::trace!("[mmu] add_write_hook: start={:#x}, end={:#x}", start, end);
        self.tlb.clear();
    ...
    pub fn add_read_hook(&mut self, start: u64, end: u64, hook: Box<dyn ReadHook>) -> Option<u32> {
        tracing::trace!("[mmu] add_read_hook: start={:#x}, end={:#x}", start, end);
        self.tlb.clear();
    ...
    pub fn map_physical(&mut self, addr: u64, index: physical::Index) -> bool {
        tracing::debug!("[mmu] map_physical: addr={:#x}, index={:?}", addr, index);
        self.map_memory_len(
            addr,
            self.page_size(),
