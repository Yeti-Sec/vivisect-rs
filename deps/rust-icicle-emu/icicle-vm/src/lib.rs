...
impl Vm {
    ...
    pub fn run(&mut self) -> VmExit {
        tracing::trace!("[vm] run: pc={:#x}, icount={}, icount_limit={}", self.cpu.read_pc(), self.cpu.icount, self.icount_limit);
    ...
    fn handle_exception(&mut self) -> VmExit {
        ...
        tracing::trace!("[vm] exception dispatch: pc={:#x}, code={:?}, value={:#x}", self.cpu.read_pc(), code, self.cpu.exception.value);
    ...
    fn handle_code_not_translated(&mut self) -> VmExit {
        ...
        tracing::debug!("[vm] code_not_translated: pc={:#x}, attempting lift", pc);
    ...
    fn run_block_interpreter(&mut self) {
        ...
        tracing::trace!("[vm] interpreter_enter: block_id={}, offset={}, pc={:#x}", block_id, offset, self.cpu.read_pc());
        if TRACE_EXEC {
    ...
    fn run_block_jit(&mut self) {
        ...
        tracing::trace!("[vm] jit_enter: pc={:#x}, fuel={}", next_addr, self.cpu.fuel.remaining);
        if TRACE_EXEC {
    ...
    pub fn lift(&mut self, addr: u64) -> Result<lifter::BlockGroup, DecodeError> {
        self.update_context();
        tracing::trace!("[vm] lift: addr={:#x}", addr);
...
impl Vm {
    pub fn step(&mut self, steps: u64) -> VmExit {
        tracing::trace!("[vm] step: steps={}, pc={:#x}, icount={}", steps, self.cpu.read_pc(), self.cpu.icount);
    ...
    pub fn add_breakpoint(&mut self, addr: u64) -> bool {
        tracing::trace!("[vm] add_breakpoint: addr={:#x}", addr);
    ...
    pub fn remove_breakpoint(&mut self, addr: u64) -> bool {
        tracing::trace!("[vm] remove_breakpoint: addr={:#x}", addr);
    ...
    pub fn snapshot(&mut self) -> Snapshot {
        tracing::trace!("[vm] snapshot: pc={:#x}, icount={}", self.cpu.read_pc(), self.cpu.icount);
        Snapshot {
