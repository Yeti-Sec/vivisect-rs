...
impl Cpu {
    ...
    pub fn block_exit(&mut self, exit: BlockExit) -> Target {
        tracing::trace!("[cpu] block_exit: pc={:#x}", self.read_pc());
        match exit {
    ...
    pub fn push_shadow_stack(&mut self, addr: u64) {
        tracing::trace!("[cpu] push_shadow_stack: return_addr={:#x}, depth={}", addr, self.shadow_stack.offset);
    ...
    pub fn pop_shadow_stack(&mut self, target: u64) {
        tracing::trace!("[cpu] pop_shadow_stack: target={:#x}, depth={}", target, self.shadow_stack.offset);
    ...
    fn load_mem<const N: usize>(&mut self, id: pcode::MemId, addr: u64) -> Option<[u8; N]> {
        match id {
            pcode::RAM_SPACE => match self.cpu.mem.read::<N>(addr, perm::READ) {
                Ok(val) => Some(val),
                Err(err) => {
                    tracing::trace!("[cpu] load_mem FAULT: addr={:#x}, space=RAM", addr);
        ...
        match id {
            pcode::RAM_SPACE => {
                if let Err(err) = self.cpu.mem.write(addr, value, perm::WRITE) {
                    tracing::trace!("[cpu] store_mem FAULT: addr={:#x}, space=RAM", addr);
...
impl Cpu {
    pub fn snapshot(&self) -> Box<CpuSnapshot> {
        tracing::trace!("[cpu] snapshot: pc={:#x}, icount={}", self.read_pc(), self.icount);
        Box::new(CpuSnapshot {
    ...
    pub fn restore(&mut self, snapshot: &CpuSnapshot) {
        tracing::trace!("[cpu] restore: pc={:#x}, icount={}", self.read_pc(), self.icount);
