    ...
    E: PcodeExecutor,
{
    ...
    match (stmt.op, output.size, (a.size(), b.size())) {
        ...
        (Op::Hook(id), ..) => {
            tracing::trace!("[interp] hook: id={}", id);
            exec.call_hook(id);
        }
        (Op::HookIf(id), ..) => {
        ...
        (Op::InstructionMarker, 0, (8, 8)) => {
            tracing::trace!("[interp] insn_marker: addr={:#x}, len={}", stmt.inputs.get()[0].as_u64(), stmt.inputs.get()[1].as_u64());
...
fn load<E: PcodeExecutor>(exec: &mut E, id: MemId, dst: VarNode, addr: u64) {
    tracing::trace!("[interp] load: space={}, addr={:#x}, size={}", id, addr, dst.size);
    macro_rules! load {
...
fn store<E: PcodeExecutor>(exec: &mut E, id: MemId, addr: u64, value: Value) {
    tracing::trace!("[interp] store: space={}, addr={:#x}, size={}", id, addr, value.size());
    macro_rules! writer {
