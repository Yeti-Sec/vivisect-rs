        ...
        for (pattern, reg_name) in THUNK_PATTERNS {
            if bytes.len() >= pattern.len() && &bytes[..pattern.len()] == *pattern {
                ...
                if let Some(func) = workspace.get_function_mut(func_va) {
                    func.meta.insert("ThunkReg".to_string(), reg_name.to_string());
...
mod tests {
    ...
    fn test_thunk_patterns_valid() {
        ...
        for (pattern, reg) in THUNK_PATTERNS {
            ...
            assert_eq!(pattern[0], 0x8B, "Pattern for {} should start with MOV opcode", reg);
            assert_eq!(pattern[2], 0x24, "Pattern for {} should have [esp] encoding", reg);
    ...
    fn test_thunk_patterns_unique_registers() {
        ...
        assert_eq!(regs.len(), unique.len(), "All register names should be unique");
    }
