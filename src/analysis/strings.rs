...
pub fn detect_utf16le_string(data: &[u8], offset: usize, min_len: usize) -> Option<DetectedString> {
    ...
    let size = (pos - offset) + if pos + 1 < data.len() && data[pos] == 0 && data[pos + 1] == 0 {
...
pub fn find_strings(data: &[u8], config: &StringAnalysisConfig) -> Vec<DetectedString> {
    ...
    while offset < data.len() {
        ...
        if !found && config.detect_utf16 && offset + 1 < data.len() {
            if data[offset + 1] == 0 && is_printable_ascii(data[offset]) {
        ...
        if !found && config.detect_utf16be && offset + 1 < data.len() {
            if data[offset] == 0 && is_printable_ascii(data[offset + 1]) {
...
mod tests {
    ...
    fn test_find_strings_all_encodings() {
        ...
        assert!(strings.iter().any(|s| s.content == "BBBB" && s.encoding == StringEncoding::Utf16Be));
    }
