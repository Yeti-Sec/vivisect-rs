...
pub fn analyze_elfplt(workspace: &mut VivWorkspace) -> VivResult<ElfPltStats> {
    ...
    for (plt_va, plt_size) in &plt_sections {
        ...
        while entry_va + entry_size as u64 <= plt_end {
            ...
            if let Some((got_target, import_name)) = resolve_plt_entry(
                workspace,
                &disasm,
                entry_va,
                arch,
...
fn resolve_plt_entry(
    ...
    if let Ok(got_bytes) = workspace.read_memory(got_va, ptr_size) {
        let target = if is_little {
            match ptr_size {
                4 => u32::from_le_bytes([got_bytes[0], got_bytes[1], got_bytes[2], got_bytes[3]])
                    as u64,
                8 => u64::from_le_bytes([
                    got_bytes[0],
                    got_bytes[1],
            ...
            match ptr_size {
                4 => u32::from_be_bytes([got_bytes[0], got_bytes[1], got_bytes[2], got_bytes[3]])
                    as u64,
                8 => u64::from_be_bytes([
                    got_bytes[0],
                    got_bytes[1],
