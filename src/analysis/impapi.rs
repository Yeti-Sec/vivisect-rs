...
pub fn lookup_api<'a>(db: &'a HashMap<String, ApiDef>, name: &str) -> Option<&'a ApiDef> {
    ...
    let basename = key.strip_prefix("thunk_")?;

    static LIBS: &[&str] = &[
        "kernel32", "ntdll", "msvcrt", "user32", "advapi32", "gdi32", "ole32", "rpcrt4", "ws2_32",
        "shell32", "msvcr71", "msvcr80", "msvcr90", "msvcr100", "msvcr110", "msvcr120",
    ];
    for lib in LIBS {
...
fn apply_api(workspace: &mut VivWorkspace, func_va: u64, api: &ApiDef) {
    if let Some(meta) = workspace.get_function_mut(func_va) {
        ...
        meta.args = api
            ...
            .enumerate()
            .map(|(i, arg)| {
                let name = arg.name.clone().unwrap_or_else(|| format!("arg{}", i));
