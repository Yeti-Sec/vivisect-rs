...
pub fn discover_go_functions(workspace: &mut VivWorkspace) -> VivResult<Vec<u64>> {
    ...
    match magic {
        ...
        GO_PCLNTAB_MAGIC_12 => parse_go_12(workspace, pclntab_va, pclntab_size, text_va, ptr_size),
        _ => {
            tracing::debug!(
...
fn parse_go_122(
    ...
    let functab_size = nfunc.saturating_mul(8);
    let functab_end = pcln_off.saturating_add(functab_size);
...
fn parse_go_116(
    ...
    for i in 0..nfunc {
        let func_va = if ptr_size == 8 {
            u64::from_le_bytes(functab[i * 16..i * 16 + 8].try_into().unwrap_or([0; 8]))
        } else {
            u32::from_le_bytes(functab[i * 8..i * 8 + 4].try_into().unwrap_or([0; 4])) as u64
        };
...
fn parse_go_12(
    ...
    let functab_size = nfunc
        ...
        .and_then(|v| v.checked_mul(ptr_size))
        .unwrap_or(usize::MAX);
    let functab_end = header_size.saturating_add(functab_size);
    if functab_end > pclntab_size {
        return Ok(Vec::new());
    }
    ...
    fn test_magic_values_high_bytes() {
        for &magic in &[
            ...
            assert_eq!(
                magic >> 8,
                0xFFFFFF,
                "Magic {:#010x} doesn't have expected high bytes",
                magic
            );
        }
    }
