...
fn hex_decode(hex: &str) -> Vec<u8> {
    (0..hex.len())
        .step_by(2)
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16).expect("MSVC signature constants are valid hex")
        })
        .collect()
}
...
pub fn analyze_msvc_function(workspace: &mut VivWorkspace, func_va: u64) -> bool {
    ...
    if let Some(sig_name) = match_vamp_signature(&bytes) {
        let short_name = sig_name.split('.').next_back().unwrap_or(sig_name);
        ...
        if let Some(meta) = workspace.get_function_mut(func_va) {
            meta.meta.insert("Thunk".to_string(), sig_name.to_string());
        }
...
mod tests {
    ...
    fn test_find_block_containing() {
        let blocks = vec![(0x401000, 20), (0x401014, 10), (0x401100, 50)];
