...
const SIGNATURES: &[CryptoSignature] = &[
    CryptoSignature { name: "AES_SBOX", pattern: AES_SBOX, min_match: 16 },
    CryptoSignature { name: "AES_INV_SBOX", pattern: AES_INV_SBOX, min_match: 16 },
    CryptoSignature { name: "SHA256_K", pattern: SHA256_K, min_match: 16 },
    CryptoSignature { name: "SHA256_H", pattern: SHA256_H, min_match: 8 },
    CryptoSignature { name: "MD5_T", pattern: MD5_T, min_match: 16 },
    CryptoSignature { name: "CRC32_TABLE", pattern: CRC32_TABLE, min_match: 16 },
    CryptoSignature { name: "BLOWFISH_P", pattern: BLOWFISH_P, min_match: 16 },
];
            ...
            for offset in 0..=(data.len() - sig.min_match) {
                ...
                if window == &sig.pattern[..sig.min_match] {
                    ...
                    if workspace.get_location(va).is_none() {
                        workspace.add_location(va, sig.pattern.len(), LocationType::Struct, Some(sig.name.to_string()));
    ...
    fn test_signatures_min_match_not_larger_than_pattern() {
        for sig in SIGNATURES {
            assert!(
                ...
                sig.name, sig.min_match, sig.pattern.len()
            );
        }
    }
