...
pub mod emu {
    ...
    pub fn create_validator_vm(workspace: &VivWorkspace) -> Option<icicle_vm::Vm> {
        ...
        // Map workspace memory regions into the VM
        for (base, size, perms, _name) in vm_memory_maps(workspace) {
            let icicle_perm = translate_perms(perms);
            vm.cpu
                .mem
                .map_memory_len(base, size, Mapping { perm: icicle_perm, value: 0 });

            // Copy data from workspace
            if let Ok(data) = workspace.read_memory(base, size as usize) {
                let _ = vm.cpu.mem.write_bytes(base, &data, perm::NONE);
            }
        }
    ...
    /// Collect workspace memory maps as (base, size, perms, name).
    fn vm_memory_maps(
        workspace: &VivWorkspace,
    ) -> Vec<(u64, u64, MemoryPermissions, String)> {
        workspace
            .get_segments()
            .iter()
            .map(|seg| {
                let perms = workspace
                    .get_permissions(seg.va)
                    .unwrap_or(MemoryPermissions::READ);
                (seg.va, seg.size as u64, perms, seg.name.clone())
            })
            .collect()
    }
    ...
    pub fn validate_with_emulation(
        vm: &mut icicle_vm::Vm,
        va: u64,
        ptr_size: usize,
    ) -> bool {
        ...
        match ptr_size {
            4 => {
                ...
                let _ = vm.cpu.mem.write_bytes(sp_after_push, &sentinel_bytes, perm::NONE);
            }
            8 => {
...
mod disasm_validator {
    ...
    pub fn validate_code_candidate(
        ...
        if insn_count >= 4 {
            for (_mnem, &count) in &mnem_counts {
        ...
        for va in candidates {
            ...
            let valid = {
                ...
                if insn_count < 1 || !has_ret {
                    ...
                    !mnem_counts.values().any(|&c| (c as f64 / insn_count as f64) >= 0.67)
                } else {
                    true
                }
            };
