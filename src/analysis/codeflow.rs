...
impl CodeFlowAnalyzer {
    ...
    pub fn analyze(&mut self, workspace: &mut VivWorkspace) -> VivResult<AnalysisResult> {
        ...
        while let Some(va) = self.pending.pop_front() {
            ...
            instruction_count += 1;
            crate::trace::emit(
                crate::trace::EventKind::DecoderDecode,
                Some(va),
                None,
                || format!("{} size={}", op.mnem, op.size),
            );
            ...
            for (target, flags) in op.get_branches() {
                crate::trace::emit(crate::trace::EventKind::BranchEmit, Some(va), None, || {
                    format!("branch -> {:?} flags={:?}", target, flags)
                });
                if let Some(target_va) = target {
                    ...
                    if flags.contains(BranchFlags::PROC)
                        && flags.contains(BranchFlags::DEREF)
                        && workspace.is_noreturn_va(target_va)
                    {
                        call_is_noreturn = true;
                    }
                    ...
                    if (flags.contains(BranchFlags::COND) || !flags.contains(BranchFlags::FALL))
                        && !flags.contains(BranchFlags::FALL)
                        && !flags.contains(BranchFlags::DEREF)
            ...
            if op.is_branch() && !op.is_call() && !op.falls_through() {
                ...
                if !has_xrefs {
                    if let Some(targets) = try_resolve_jump_table(workspace, &disasm, va, &op) {
            ...
            // Track data references from LEA/MOV instructions that reference
            // non-code addresses. This catches patterns like:
            //   lea rax, [rip+0x1234]  → data reference to string/global
            //   mov rax, [rip+0x5678]  → data reference to pointer/variable
            for operand_addr in op.get_memory_references() {
                if workspace.is_valid_pointer(operand_addr)
                    && !self.functions.contains(&operand_addr)
                    && !self.block_starts.contains(&operand_addr)
                {
                    workspace.add_xref(va, operand_addr, RefType::Data);
                }
            }
...
#[must_use]
pub fn analyze_function(workspace: &mut VivWorkspace, func_va: u64) -> VivResult<FunctionAnalysis> {
    let arch = workspace.architecture();
    ...
    while let Some(start) = todo.pop() {
        ...
        loop {
            ...
            if op.is_call() {
                ...
                let is_noreturn_call = targets
                    .iter()
                    .any(|&(tva, _)| workspace.is_noreturn_va(tva));
                if is_noreturn_call {
                    if block_instr_count > 0 {
                        blocks.push(BasicBlockInfo {
                            start,
...
pub fn scan_function_prologues(workspace: &mut VivWorkspace) -> VivResult<Vec<u64>> {
    ...
    let _min_pattern_len = patterns.iter().map(|p| p.len()).min().unwrap_or(3);
    ...
    for (seg_va, seg_size) in &code_segments {
        ...
        for pattern in &patterns {
            ...
            for offset in 0..=(seg_bytes.len() - pattern.len()) {
                ...
                let valid =
                    validate_prologue_candidate(&disasm, &seg_bytes[offset..], candidate_va);
...
fn validate_prologue_candidate(disasm: &X86Disassembler, bytes: &[u8], start_va: u64) -> bool {
    let mut va = start_va;
    ...
    for i in 0..max_entries {
        ...
        let target = match ptr_size {
            4 => u32::from_le_bytes(
                entry_bytes[..4]
                    .try_into()
                    .expect("read_memory returned exactly ptr_size bytes"),
            ) as u64,
            8 => u64::from_le_bytes(
                entry_bytes[..8]
                    .try_into()
