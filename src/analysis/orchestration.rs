                ...
                *total_funcs += 1;
                ...
                for fmod in func_modules.iter_mut() {
                    ...
                    match fmod.analyze_function(workspace, func_va) {
                        Ok(stats) => {
                            result.add_module(format!("{}@{:#x}", fname, func_va), stats);
                        }
                        Err(e) => {
                            result.add_failure(format!("{}@{:#x}", fname, func_va), e.to_string());
                        }
                    }
                }
            }
...
impl AnalysisModule for StringAnalysis {
    ...
    fn analyze(&mut self, workspace: &mut VivWorkspace) -> VivResult<AnalysisStats> {
        use crate::analysis::strings::{detect_ascii_string, detect_utf16le_string};
        ...
        let segments: Vec<(u64, usize)> = workspace
            ...
            .filter(|seg| {
                ...
                !name_lower.contains("text") && !name_lower.contains("code") && seg.size > 0
            })
        ...
        let lookup_name = {
            ...
            if let Some(name) = thunk_name {
                ...
                workspace
                    .get_name(func_va)
                    .map(|s| s.to_string())
                    .or_else(|| workspace.get_function(func_va).and_then(|m| m.name.clone()))
            }
        };
            ...
            if let Some(api) = lookup_api(db, &name) {
                if let Some(meta) = workspace.get_function_mut(func_va) {
                    ...
                    meta.args = api
                        ...
                        .enumerate()
                        .map(|(i, arg)| {
                            let arg_name = arg.name.clone().unwrap_or_else(|| format!("arg{}", i));
    ...
    fn analyze(&mut self, workspace: &mut VivWorkspace) -> VivResult<AnalysisStats> {
        ...
        match discover_go_functions(workspace) {
            Ok(funcs) => {
                ...
                tracing::debug!("[golang] discovered {} functions from pclntab", funcs.len());
            }
            Err(e) => {
...
impl AnalysisModule for ElfPltAnalysis {
    ...
    fn should_run(&self, workspace: &VivWorkspace) -> bool {
        ...
        workspace
            .get_segments()
            .iter()
            .any(|seg| seg.name.starts_with(".plt"))
    }
...
impl AnalysisModule for LibcStartMainAnalysis {
    ...
    fn should_run(&self, workspace: &VivWorkspace) -> bool {
        ...
        workspace
            .symbols()
            .iter()
            .any(|(_, name)| name.to_lowercase().contains("__libc_start_main"))
    }
...
impl AnalysisModule for ArmAnalysis {
    ...
    fn should_run(&self, ws: &VivWorkspace) -> bool {
        matches!(
            ws.architecture(),
            crate::constants::Architecture::ArmV7 | crate::constants::Architecture::Thumb
        )
    }
    fn analyze(&mut self, ws: &mut VivWorkspace) -> VivResult<AnalysisStats> {
        let mut stats = AnalysisStats::new();
...
impl AnalysisModule for ElfPltLateAnalysis {
    ...
    fn should_run(&self, ws: &VivWorkspace) -> bool {
        ws.get_segments()
            .iter()
            .any(|s| s.name.to_lowercase().starts_with(".plt"))
    }
    fn analyze(&mut self, ws: &mut VivWorkspace) -> VivResult<AnalysisStats> {
        let mut stats = AnalysisStats::new();
        ...
        fn analyze(&mut self, workspace: &mut VivWorkspace) -> VivResult<AnalysisStats> {
            ...
            for &va in &self.funcs_to_create {
                if !workspace.is_function(va) {
                    workspace.add_function(va, crate::core::workspace::FunctionMeta::default())?;
                    stats.functions_discovered += 1;
                }
            }
            Ok(stats)
