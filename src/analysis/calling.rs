...
#[cfg(feature = "icicle")]
fn analyze_with_emulation(workspace: &mut VivWorkspace) -> VivResult<Vec<CallingConvResult>> {
    ...
    for func_va in &funcs {
        ...
        // Run (the VmExit result is not needed here — we inspect the stack).
        let _ = vm.run();
...
pub fn analyze_calling_conventions(workspace: &mut VivWorkspace) -> VivResult<CallingStats> {
    ...
    let results = {
        // `arch` is only needed by the static analysis path.
        let arch = workspace.architecture();
        let funcs: Vec<u64> = workspace.get_functions();
        funcs
            .iter()
