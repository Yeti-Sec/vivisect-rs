...
pub fn analyze_elfplt_late(workspace: &mut VivWorkspace) -> VivResult<ElfPltLateStats> {
    ...
    tracing::debug!("[elfplt_late] filled {} PLT gaps", stats.functions_created);
...
fn analyze_plt_section(
    ...
    let (&func_dist, _) = dist_counts
        .iter()
        .max_by_key(|(_, &count)| count)
        .expect("dist_counts is non-empty (checked above)");
    if func_dist == 0 {
        return Ok(());
    }
    ...
    let ref_fva = if plt_funcs.len() > 1 {
        plt_funcs[1]
    } else {
        plt_funcs[0]
    };
    ...
    if let Some(ref_mnem) = &ref_mnem {
        ...
        let last_known = *plt_funcs
            .last()
            .expect("plt_funcs is non-empty at this point in the analysis");
