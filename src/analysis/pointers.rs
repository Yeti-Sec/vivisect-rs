...
fn is_probably_code(workspace: &VivWorkspace, va: u64) -> bool {
    ...
    match arch {
        crate::constants::Architecture::I386 => {
            ...
            if bytes.len() >= 5 && bytes[0] == 0x8B && bytes[1] == 0xFF
                && bytes[2] == 0x55 && bytes[3] == 0x8B && bytes[4] == 0xEC {
                return true;
            }
        ...
        crate::constants::Architecture::Amd64 => {
            ...
            if bytes.len() >= 4 && bytes[0] == 0x55 && bytes[1] == 0x48
            ...
            if bytes.len() >= 5 && bytes[0] == 0x48 && bytes[1] == 0x89
                && (bytes[2] & 0xC7) == 0x44 {
                return true;
            }
            false
        ...
        LocationType::String => {
            ...
            if let Ok(bytes) = workspace.read_memory(va, 4096) {
                if let Some(s) = detect_ascii_string(&bytes, 0, MIN_STRING_LENGTH) {
                    workspace.add_location(va, s.size, LocationType::String, Some(s.content.clone()));
                    workspace.set_name(va, &s.content);
                    tracing::debug!("[pointers] discovered string at 0x{:x}: {:?}", va, s.content);
                    return true;
                }
            }
            false
        ...
        LocationType::Unicode => {
            if let Ok(bytes) = workspace.read_memory(va, 8192) {
                if let Some(s) = detect_utf16le_string(&bytes, 0, MIN_STRING_LENGTH) {
                    workspace.add_location(va, s.size, LocationType::Unicode, Some(s.content.clone()));
                    workspace.set_name(va, &s.content);
                    tracing::debug!("[pointers] discovered unicode at 0x{:x}: {:?}", va, s.content);
                    return true;
                }
            }
            false
...
pub fn analyze_pointers(workspace: &mut VivWorkspace) -> VivResult<PointerStats> {
    ...
    for (addr, pval) in free_ptrs {
        ...
        // Follow the target
        if workspace.is_valid_pointer(pval) {
            if follow_pointer(workspace, pval) {
                stats.followed += 1;
            }
        }
...
fn read_pointer(workspace: &VivWorkspace, va: u64, size: usize, little_endian: bool) -> Option<u64> {
