...
fn disasm_at(disasm: &X86Disassembler, workspace: &VivWorkspace, va: u64) -> Option<Opcode> {
...
fn get_switch_base(
    ...
    let reg_for_mov = if add_val == Some(imgbase) {
        jmp_reg.clone()
    } else {
    ...
    if !workspace.is_valid_pointer(table_va) {
        tracing::trace!("0x{:x}: table VA {:#x} not valid", jmp_op.va, table_va);
        return None;
    }
