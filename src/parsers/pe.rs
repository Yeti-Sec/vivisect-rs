...
fn extract_additional_entries(
    ...
    if let Some(tls_dd) = tls_dir {
        if tls_dd.virtual_address != 0 {
            ...
            if let Ok(tls_bytes) =
                read_va_raw(data, sections, image_base, image_base + tls_rva, callbacks_field_offset + ptr_size)
            {
                ...
                if callbacks_va != 0 {
                    for i in 0..64u64 {
                        ...
                        if let Ok(cb_bytes) = read_va_raw(data, sections, image_base, cb_addr, ptr_size) {
                            let cb_va = if is_64 {
                                u64::from_le_bytes(cb_bytes.try_into().unwrap_or([0; 8]))
                            } else {
                                u32::from_le_bytes(cb_bytes[..4].try_into().unwrap_or([0; 4])) as u64
                            };
                            if cb_va == 0 {
                                break;
                            }
    ...
    if !text_ranges.is_empty() {
        ...
        for section in &section_infos {
            ...
            let sec_data = match read_va_raw(data, sections, image_base, section.virtual_address, section.raw_size) {
                Ok(d) => d,
                Err(_) => continue,
            };
            ...
            for offset in (0..sec_data.len().saturating_sub(ptr_size)).step_by(ptr_size) {
                let ptr_val = if ptr_size == 8 {
                    ...
                    u32::from_le_bytes(sec_data[offset..offset + 4].try_into().unwrap_or([0; 4])) as u64
                };
...
impl PeParser {
    ...
    pub fn read_va(&self, va: u64, size: usize) -> VivResult<Vec<u8>> {
        read_va_raw(&self.data, &self.sections, self.image_base, va, size)
    }
    ...
    /// Get the raw PE header bytes (up to SizeOfHeaders).
    pub fn header_bytes(&self) -> Vec<u8> {
        let size = self.header_size.min(self.data.len());
