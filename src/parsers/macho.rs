                    ...
                    if let Some(cputype) = preferred_cputype {
                        for entry in multi {
                            if let Ok(goblin::mach::SingleArch::MachO(ref macho)) = entry {
                                if macho.header.cputype as u32 == cputype {
    ...
    fn extract_fields(macho: &goblin::mach::MachO<'_>) -> ExtractedMachO {
        ...
        for segment in &macho.segments {
            ...
            for section_result in segment {
                if let Ok((section, _section_data)) = section_result {
...
mod tests {
    ...
    #[test]
    fn test_macho_read_va_invalid() {
        let data = make_minimal_macho64();
        let parser = MachOParser::from_bytes(data).unwrap();
        let result = parser.read_va(0x100000000, 4);
        assert!(result.is_err());
    }
    ...
    fn test_macho_magic_detection() {
        ...
        assert_eq!(detect_format_bytes(&macho64_le).unwrap(), BinaryFormat::MachO);
        let macho64_be = [0xFE, 0xED, 0xFA, 0xCF];
        assert_eq!(detect_format_bytes(&macho64_be).unwrap(), BinaryFormat::MachO);
        let macho32_le = [0xCE, 0xFA, 0xED, 0xFE];
        assert_eq!(detect_format_bytes(&macho32_le).unwrap(), BinaryFormat::MachO);
        let macho32_be = [0xFE, 0xED, 0xFA, 0xCE];
        assert_eq!(detect_format_bytes(&macho32_be).unwrap(), BinaryFormat::MachO);
