...
impl WorkspaceEmulator {
    ...
    fn init_default_stubs(&mut self) {
        ...
        for name in &[
            "exitprocess", "exit", "_exit", "abort", "terminateprocess",
            "exitthread", "rtlexituserthread", "_cexit", "_c_exit",
            "fatalexit", "fatalappexita", "fatalappexitw",
        ] {
            self.api_stubs.insert(name.to_string(), ApiStubAction::StopExecution);
        }

        // Memory allocation → return non-null (stack-based fake ptr)
        for name in &[
            "malloc", "calloc", "realloc", "virtualalloc", "virtualallocex",
            "heapalloc", "localalloc", "globalalloc",
            "cotaskmemalloc", "sysstringlen",
        ] {
            self.api_stubs.insert(name.to_string(), ApiStubAction::ReturnOne);
        }

        // Free functions → return success
        for name in &[
            "free", "virtualfree", "heapfree", "localfree", "globalfree",
            "cotaskmemfree",
        ] {
            self.api_stubs.insert(name.to_string(), ApiStubAction::ReturnZero);
        }

        // Handle/success functions → return non-zero
        for name in &[
            "getprocessheap", "getmodulehandlea", "getmodulehandlew",
            "loadlibrarya", "loadlibraryw", "loadlibraryexa", "loadlibraryexw",
            "getprocaddress", "createfilea", "createfilew",
            "openprocess", "createmutexa", "createmutexw",
    ...
    pub fn run_function(
        ...
        let disasm = match arch {
            ...
            _ => return Err(VivError::AnalysisError {
                ...
                if target != 0 {
                    ...
                    if let Some(action) = self.get_stub_action(stub_name) {
                        match action {
                            ...
                            ApiStubAction::ReturnZero | ApiStubAction::ReturnOne | ApiStubAction::Skip => {
    ...
    pub fn run_function_icicle(
        ...
        let mut vm = emu::create_validator_vm(workspace)
            .ok_or_else(|| VivError::AnalysisError {
                message: format!("unsupported architecture: {:?}", arch),
            })?;
        ...
        let returned = matches!(exit, icicle_cpu::VmExit::Breakpoint)
            && vm.cpu.read_pc() == SENTINEL_ADDR;
...
mod tests {
    ...
    fn test_get_stub_action_case_insensitive() {
        ...
        assert_eq!(emu.get_stub_action("myfunc"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("MYFUNC"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("MyFunc"), Some(ApiStubAction::ReturnOne));
    }
    ...
    fn test_default_stubs_noreturn() {
        ...
        assert_eq!(emu.get_stub_action("exitprocess"), Some(ApiStubAction::StopExecution));
        assert_eq!(emu.get_stub_action("exit"), Some(ApiStubAction::StopExecution));
        assert_eq!(emu.get_stub_action("_exit"), Some(ApiStubAction::StopExecution));
        assert_eq!(emu.get_stub_action("abort"), Some(ApiStubAction::StopExecution));
        assert_eq!(emu.get_stub_action("terminateprocess"), Some(ApiStubAction::StopExecution));
    }
    ...
    fn test_default_stubs_memory_alloc() {
        ...
        assert_eq!(emu.get_stub_action("malloc"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("calloc"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("virtualalloc"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("heapalloc"), Some(ApiStubAction::ReturnOne));
    }
    ...
    fn test_default_stubs_free() {
        ...
        assert_eq!(emu.get_stub_action("virtualfree"), Some(ApiStubAction::ReturnZero));
        assert_eq!(emu.get_stub_action("heapfree"), Some(ApiStubAction::ReturnZero));
    }
    ...
    fn test_default_stubs_handle_functions() {
        let emu = WorkspaceEmulator::new();
        // Handle/success functions return non-zero
        assert_eq!(emu.get_stub_action("getprocessheap"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("loadlibrarya"), Some(ApiStubAction::ReturnOne));
        assert_eq!(emu.get_stub_action("getprocaddress"), Some(ApiStubAction::ReturnOne));
    }
    ...
    fn test_default_stubs_query_functions() {
        let emu = WorkspaceEmulator::new();
        // Query functions return zero (no error)
        assert_eq!(emu.get_stub_action("getlasterror"), Some(ApiStubAction::ReturnZero));
        assert_eq!(emu.get_stub_action("closehandle"), Some(ApiStubAction::ReturnZero));
        assert_eq!(emu.get_stub_action("sleep"), Some(ApiStubAction::ReturnZero));
    }
