...
async fn get_info(State(ws): State<SharedWorkspace>) -> Json<InfoResponse> {
    ...
    Json(InfoResponse {
        ...
        entry_points: ws.entry_points().iter().map(|va| format!("{:#x}", va)).collect(),
    })
}

async fn get_functions(State(ws): State<SharedWorkspace>) -> Json<Vec<FunctionEntry>> {
    let funcs: Vec<FunctionEntry> = ws.get_functions().iter().map(|&va| {
...
async fn get_function(
    ...
    let blocks: Vec<BlockEntry> = ws.get_function_blocks(va).iter().map(|b| BlockEntry {
        va: format!("{:#x}", b.va),
        size: b.size,
    }).collect();
...
async fn get_segments(State(ws): State<SharedWorkspace>) -> Json<Vec<SegmentEntry>> {
    let segs: Vec<SegmentEntry> = ws.get_segments().iter().map(|s| SegmentEntry {
        va: format!("{:#x}", s.va),
        size: s.size,
        name: s.name.clone(),
    }).collect();
...
async fn get_xrefs_to(
    ...
    let va = parse_va(&va_str).ok_or(StatusCode::BAD_REQUEST)?;
    let xrefs: Vec<XrefEntry> = ws.get_xrefs_to(va).iter().map(|(from_va, ref_type)| {
        XrefEntry {
...
async fn get_xrefs_from(
    ...
    let va = parse_va(&va_str).ok_or(StatusCode::BAD_REQUEST)?;
    let xrefs: Vec<XrefEntry> = ws.get_xrefs_from(va).iter().map(|(to_va, ref_type)| {
        XrefEntry {
            from_va: format!("{:#x}", va),
...
async fn get_names(State(ws): State<SharedWorkspace>) -> Json<Vec<NameEntry>> {
    let names: Vec<NameEntry> = ws.symbols().iter().map(|(va, name)| NameEntry {
        va: format!("{:#x}", va),
        name: name.to_string(),
    }).collect();
    Json(names)
...
async fn get_strings(State(ws): State<SharedWorkspace>) -> Json<Vec<StringEntry>> {
    let strings: Vec<StringEntry> = ws.locations_iter()
...
async fn get_memory(
    ...
    let data = ws.read_memory(va, size).map_err(|_| StatusCode::NOT_FOUND)?;
