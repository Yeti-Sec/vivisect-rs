...
impl Opcode {
    ...
    pub fn get_branches(&self) -> Vec<(Option<u64>, BranchFlags)> {
        ...
        // Only extract operand-based targets for branch/call/return instructions.
        // Non-branch instructions (mov, add, etc.) should NOT have their operand
    ...
    pub fn get_memory_references(&self) -> Vec<u64> {
        ...
        for oper in &self.opers {
            if let Some(addr) = oper.get_value(self) {
                if addr > 0x1000 {
                    refs.push(addr);
                }
            }
