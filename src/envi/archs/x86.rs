...
use crate::envi::operand::{
    DerefOperand, ImmediateOperand, Operand, PcRelativeOperand, RegisterOperand,
};
...
impl X86Disassembler {
    ...
    fn instruction_to_opcode(&self, instr: &Instruction, bytes: &[u8]) -> VivResult<Opcode> {
        ...
        for i in 0..instr.op_count() {
            if let Some(oper) = self.convert_operand(instr, i) {
                operands.push(oper);
            }
        }
    ...
    fn convert_operand(&self, instr: &Instruction, op_idx: u32) -> Option<Box<dyn Operand>> {
        ...
        match op_kind {
            ...
            OpKind::Immediate8 | OpKind::Immediate8_2nd => Some(Box::new(ImmediateOperand::new(
                instr.immediate8() as u64,
                1,
            ))),
            OpKind::Immediate16 => Some(Box::new(ImmediateOperand::new(
                instr.immediate16() as u64,
                2,
            ))),
            OpKind::Immediate32 => Some(Box::new(ImmediateOperand::new(
                instr.immediate32() as u64,
                4,
            ))),
            OpKind::Immediate64 => Some(Box::new(ImmediateOperand::new(instr.immediate64(), 8))),
            OpKind::Immediate8to16 | OpKind::Immediate8to32 | OpKind::Immediate8to64 => {
                Some(Box::new(ImmediateOperand::signed(
                    instr.immediate8to64(),
                    self.mode.pointer_size(),
                )))
            }
            OpKind::Immediate32to64 => Some(Box::new(ImmediateOperand::signed(
                instr.immediate32to64(),
                8,
            ))),
            OpKind::NearBranch16 => {
            ...
            OpKind::Memory => {
                ...
                let index = if index_reg != Register::None {
                    Some((
                        index_reg as usize,
                        format!("{:?}", index_reg).to_lowercase(),
                    ))
                } else {
                    None
                };

                Some(Box::new(DerefOperand::sib(
                    base,
                    index,
                    scale as u8,
                    disp,
...
impl ArchitectureModule for X86Disassembler {
    ...
    fn get_emulator(&self) -> VivResult<Box<dyn crate::abstractions::Emulator>> {
        Err(VivError::Other {
            message: "x86 emulator: use CpuEmulator trait via vivutils-rs".into(),
        })
    }
...
fn memory_size_bytes(size: iced_x86::MemorySize) -> usize {
    match size {
        ...
        | iced_x86::MemorySize::Float32 => 4,
        iced_x86::MemorySize::UInt64
        | iced_x86::MemorySize::Int64
        | iced_x86::MemorySize::Float64 => 8,
        iced_x86::MemorySize::UInt128
        | iced_x86::MemorySize::Int128
        | iced_x86::MemorySize::Float128 => 16,
