...
impl RiscvDisassembler {
    pub fn new(mode: RiscvMode) -> Self {
        Self { mode }
    }
}
...
impl crate::envi::ArchitectureModule for RiscvDisassembler {
    fn arch_id(&self) -> Architecture {
        match self.mode {
            RiscvMode::Rv32 => Architecture::RiscV32,
            RiscvMode::Rv64 => Architecture::RiscV64,
        }
    }
    fn arch_name(&self) -> &str {
        match self.mode {
            RiscvMode::Rv32 => "riscv32",
            RiscvMode::Rv64 => "riscv64",
        }
    }
    ...
    fn break_instruction(&self) -> Option<&[u8]> {
        Some(&[0x73, 0x00, 0x10, 0x00])
    }
    fn nop_instruction(&self) -> Option<&[u8]> {
        Some(&[0x13, 0x00, 0x00, 0x00])
    }

    fn parse_opcode(&self, _bytes: &[u8], _offset: usize, _va: u64) -> VivResult<Opcode> {
        Err(VivError::Other {
            message: "RISC-V disassembly requires icicle Sleigh backend".into(),
        })
    }

    fn get_emulator(&self) -> VivResult<Box<dyn crate::abstractions::Emulator>> {
        Err(VivError::Other {
            message: "RISC-V emulator: use CpuEmulator trait via vivutils-rs".into(),
        })
    }
