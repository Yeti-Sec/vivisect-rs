...
use crate::constants::{Architecture, Endian, InstructionFlags};
...
impl ArmDisassembler {
    ...
    pub fn disassemble(&self, bytes: &[u8], va: u64) -> VivResult<Opcode> {
        ...
        let insns = cs
            ...
            .map_err(|_| VivError::InvalidInstruction { address: va })?;
        ...
        if let Some(op_str) = insn.op_str() {
            for part in op_str.split(',') {
                    ...
                    || part == "fp"
                {
                    ...
                    operands.push(Box::new(RegisterOperand::new(
                        reg_num,
                        part.to_string(),
                        size,
                    )));
