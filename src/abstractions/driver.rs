    ...
    pub fn run(&mut self) -> VivResult<(usize, DriverStopReason)> {
        ...
        while self.instructions_executed < self.max_instructions {
            ...
            if self.emu.is_breakpoint(pc) && self.instructions_executed > 0 {
                return Ok((self.instructions_executed, DriverStopReason::Breakpoint(pc)));
            }
...
mod tests {
    ...
    impl MockEmulator {
        fn new(start: u64, size: u64) -> Self {
            ...
            let _ = rax;
            regs.set_register("rip", start)
                .expect("rip register was just added");
            regs.set_register("rsp", 0x7FFE0000)
                .expect("rsp register was just added");
    ...
    impl crate::abstractions::MemoryMap for MockEmulator {
        ...
        fn get_memory_map(
            &self,
            addr: u64,
        ) -> Option<(u64, usize, MemoryPermissions, Option<String>)> {
            self.memory
                .iter()
                .find(|r| r.contains(addr))
                .map(|r| (r.base, r.size, r.permissions, r.name.clone()))
    ...
    impl crate::abstractions::RegisterContext for MockEmulator {
        ...
        fn get_register_info_by_index(
            &self,
            index: usize,
        ) -> Option<crate::abstractions::RegisterInfo> {
    ...
    fn test_driver_run_invalid_instruction() {
        ...
        assert!(matches!(
            reason,
            DriverStopReason::InvalidInstruction(0x1005)
        ));
    }
