...
impl XRefManager {
    ...
    /// Add a cross-reference. Deduplicates — adding the same (from, to, type) twice is a no-op.
    pub fn add_xref(&mut self, from: u64, to: u64, ref_type: RefType) {
        let to_refs = self.to_map.entry(to).or_default();
        if to_refs.iter().any(|&(f, t)| f == from && t == ref_type) {
            return;
        }
        to_refs.push((from, ref_type));
    ...
    pub fn iter(&self) -> impl Iterator<Item = (u64, u64, RefType)> + '_ {
        self.from_map
            .iter()
            .flat_map(|(&from, refs)| refs.iter().map(move |&(to, ref_type)| (from, to, ref_type)))
    }
...
mod tests {
    ...
    #[test]
    fn test_duplicate_xrefs_are_deduplicated() {
        let mut xrefs = XRefManager::new();
        // Adding the exact same xref twice should deduplicate (matches Python vivisect)
        xrefs.add_xref(0x1000, 0x2000, RefType::Code);
        xrefs.add_xref(0x1000, 0x2000, RefType::Code);
        assert_eq!(xrefs.len(), 1);
    ...
    #[test]
    fn test_dedup_allows_different_types_same_addresses() {
        let mut xrefs = XRefManager::new();
        xrefs.add_xref(0x1000, 0x2000, RefType::Code);
        xrefs.add_xref(0x1000, 0x2000, RefType::Data);
        // Different ref types for same (from, to) pair should both be stored
        assert_eq!(xrefs.len(), 2);
    }

    #[test]
    fn test_dedup_triple_insert() {
        let mut xrefs = XRefManager::new();
        xrefs.add_xref(0x1000, 0x2000, RefType::Code);
        xrefs.add_xref(0x1000, 0x2000, RefType::Code);
        xrefs.add_xref(0x1000, 0x2000, RefType::Code);
