...
impl ControlFlowGraph {
    ...
    pub fn find_paths(&self, from_va: u64, to_va: u64, max_paths: usize) -> Vec<Vec<u64>> {
        ...
        while let Some((current, path)) = stack.pop() {
            ...
            if current == to_idx {
                let va_path: Vec<u64> = path.iter()
                    .map(|&idx| self.graph[idx].start)
                    .collect();
                paths.push(va_path);
                visited_paths += 1;
                continue;
    ...
    pub fn find_loop_headers(&self) -> Vec<u64> {
        ...
        let order_map: HashMap<NodeIndex, usize> = post_order.iter()
            .enumerate()
            .map(|(i, &idx)| (idx, i))
            .collect();
        ...
        for edge in self.graph.edge_indices() {
            if let Some((src, tgt)) = self.graph.edge_endpoints(edge) {
                if let (Some(&src_order), Some(&tgt_order)) = (order_map.get(&src), order_map.get(&tgt)) {
    ...
    pub fn to_serializable(&self) -> SerializableCfg {
        let nodes: Vec<(u64, usize)> = self.node_map.iter()
            .map(|(&va, &idx)| (va, self.graph[idx].size))
            .collect();

        let edges: Vec<(u64, u64, String)> = self.graph.edge_indices()
            .filter_map(|eidx| {
