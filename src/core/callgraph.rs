...
impl CallGraph {
    ...
    pub fn add_call(&mut self, caller: u64, callee: u64, call_site: u64, is_direct: bool) {
...
mod tests {
    ...
    fn test_build_with_lookup_interval_search() {
        ...
        let intervals: Vec<(u64, u64, u64)> =
            vec![(0x1000, 0x1100, 0x1000), (0x2000, 0x2080, 0x2000)];
    ...
    fn test_build_with_lookup_no_self_calls() {
        ...
        let builder = CallGraphBuilder::new(&xrefs, &functions);
        let lookup = |addr: u64| -> Option<u64> {
            if (0x1000..0x1100).contains(&addr) {
                Some(0x1000)
            } else {
                None
            }
