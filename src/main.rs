...
fn cmd_analyze(file: &PathBuf, output: Option<&std::path::Path>, verbose: bool) -> VivResult<()> {
    if verbose {
    ...
    // Actually run the analysis pipeline (previously this only loaded).
    workspace.analyze();
    ...
    // Persist the analyzed workspace when an output path is given, so the
    // documented analyze -> save -> reload -> functions workflow works
    // (review finding #13).
    if let Some(out) = output {
        vivisect::storage::msgpack::save_workspace(&workspace, out)?;
        println!("Saved workspace: {}", out.display());
    }
...
fn cmd_functions(file: &PathBuf) -> VivResult<()> {
    // Accept either a saved workspace (msgpack) or a raw binary. A saved
    // workspace reloads its analyzed function set without touching the original
    // binary; a raw binary is loaded and analyzed on the fly (finding #13).
    let workspace = match vivisect::storage::msgpack::load_workspace(file) {
        Ok(ws) => ws,
        Err(_) => {
            let mut ws = VivWorkspace::new();
            ws.load_from_file(file)?;
            ws.analyze();
            ws
        }
    };

    let mut functions = workspace.get_functions();
...
fn cmd_sections(file: &PathBuf) -> VivResult<()> {
    ...
    match format {
        BinaryFormat::Pe => {
            ...
            println!(
                "{:<16} {:>16} {:>12} {:>8}",
                "Name", "VirtualAddr", "VirtualSize", "Perms"
            );
            println!("{}", "-".repeat(56));
            for s in sections {
                let perms = format!(
                ...
                println!(
                    "{:<16} {:#16x} {:>12} {:>8}",
                    s.name, s.virtual_address, s.virtual_size, perms
                );
            }
        }
        BinaryFormat::Elf => {
...
fn cmd_disasm(file: &PathBuf, address: Option<&str>, count: usize) -> VivResult<()> {
    ...
    println!("{:<16} {:<24} Instruction", "Address", "Bytes");
    println!("{}", "-".repeat(60));
    ...
    while instructions < count {
        ...
        match disasm.disassemble(&bytes, current_addr) {
            Ok(op) => {
                ...
                println!(
                    "{:#016x}: {:<24} {}{}",
                    current_addr, bytes_hex, op.mnem, operand_str
                );
