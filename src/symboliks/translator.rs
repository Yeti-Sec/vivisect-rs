...
impl SymbolikTranslator {
    ...
    pub fn eff_constrain(&mut self, target: u64, constraint: SymbolicValue) {
        self.constraints
            .push(BranchConstraint { target, constraint });
    }
    ...
    fn i_cmov(&mut self, op: &Opcode, cmp_op: ConstraintOp) {
        ...
        if let (SymbolicValue::Const { value: lv, .. }, SymbolicValue::Const { value: rv, .. }) =
            (&dst, &src)
        {
    ...
    fn i_inc(&mut self, op: &Opcode) {
        let dst = self.get_oper_obj(op, 0);
        let width = dst.width();
        let result = SymbolicValue::oper(Op::Add, dst, SymbolicValue::constant(1, width));
        self.set_oper_obj(op, 0, result);
    }
    ...
    fn i_dec(&mut self, op: &Opcode) {
        let dst = self.get_oper_obj(op, 0);
        let width = dst.width();
        let result = SymbolicValue::oper(Op::Sub, dst, SymbolicValue::constant(1, width));
        self.set_oper_obj(op, 0, result);
    }
    ...
    fn i_neg(&mut self, op: &Opcode) {
        let dst = self.get_oper_obj(op, 0);
        let result = SymbolicValue::oper(Op::Neg, dst, SymbolicValue::constant(0, 0));
        self.set_oper_obj(op, 0, result);
    }
    ...
    fn i_not(&mut self, op: &Opcode) {
        let dst = self.get_oper_obj(op, 0);
        let result = SymbolicValue::oper(Op::Not, dst, SymbolicValue::constant(0, 0));
        self.set_oper_obj(op, 0, result);
    }
    ...
    fn i_cmp(&mut self, op: &Opcode) {
        // CMP sets flags based on (v1 - v2) without storing the result.
        // Port of Python i386.py i_cmp (line 372-380):
        //   eflags_cf = gt(v2, v1)
        //   eflags_gt = gt(v1, v2)
        //   eflags_lt = lt(v1, v2)
        //   eflags_sf = lt(v1, v2)
        //   eflags_eq = eq(v1, v2)
        let v1 = self.get_oper_obj(op, 0);
        let v2 = self.get_oper_obj(op, 1);

        // Also keep internal state for Jcc resolution
        self.set_reg_obj("__cmp_lhs", v1.clone());
        self.set_reg_obj("__cmp_rhs", v2.clone());

        // Generate eflags effects matching Python
        self.eff_set_variable(
            "eflags_cf",
            SymbolicValue::constraint(ConstraintOp::Ugt, v2.clone(), v1.clone()),
        );
        self.eff_set_variable(
            "eflags_gt",
            SymbolicValue::constraint(ConstraintOp::Gt, v1.clone(), v2.clone()),
        );
        self.eff_set_variable(
            "eflags_lt",
            SymbolicValue::constraint(ConstraintOp::Lt, v1.clone(), v2.clone()),
        );
        self.eff_set_variable(
            "eflags_sf",
            SymbolicValue::constraint(ConstraintOp::Lt, v1.clone(), v2.clone()),
        ...
        self.eff_set_variable(
            "eflags_eq",
            SymbolicValue::constraint(ConstraintOp::Eq, v1, v2),
        );
    }

    /// TEST op0, op1
    fn i_test(&mut self, op: &Opcode) {
        // TEST sets flags based on (v1 & v2) without storing the result.
        // Port of Python i386.py i_test (line 1357-1372):
        //   obj = v1 & v2
        //   eflags_gt = gt(v1, v2)
        //   eflags_lt = lt(obj, 0)    (SF != OF, OF is cleared)
        //   eflags_sf = lt(obj, 0)    (v1 & v2 < 0)
        //   eflags_eq = eq(obj, 0)    (v1 & v2 == 0)
        //   eflags_of = 0
        //   eflags_cf = 0
        //   eflags_pf = xor(xor(v1, v2), -1)
        let v1 = self.get_oper_obj(op, 0);
        let v2 = self.get_oper_obj(op, 1);
        let obj = SymbolicValue::oper(Op::And, v1.clone(), v2.clone());
        let zero = SymbolicValue::constant(0, self.pointer_size);

        // Also keep internal state for Jcc resolution
        self.set_reg_obj("__cmp_lhs", obj.clone());
        self.set_reg_obj("__cmp_rhs", zero.clone());
        ...
        self.eff_set_variable(
            "eflags_lt",
            SymbolicValue::constraint(ConstraintOp::Lt, obj.clone(), zero.clone()),
        );
        self.eff_set_variable(
            "eflags_sf",
            SymbolicValue::constraint(ConstraintOp::Lt, obj.clone(), zero.clone()),
        ...
        self.eff_set_variable(
            "eflags_eq",
            SymbolicValue::constraint(ConstraintOp::Eq, obj, zero.clone()),
        );
        self.eff_set_variable("eflags_of", zero.clone());
        self.eff_set_variable("eflags_cf", zero);

        // Parity flag: xor(xor(v1, v2), -1)
        let neg_one = SymbolicValue::constant(u64::MAX, self.pointer_size);
        let pf = SymbolicValue::oper(Op::Xor, SymbolicValue::oper(Op::Xor, v1, v2), neg_one);
        self.eff_set_variable("eflags_pf", pf);
    }
    ...
    fn i_convert(&mut self, op: &Opcode) {
        ...
        let extended = SymbolicValue::oper(Op::SignExtend, src_val, SymbolicValue::constant(0, 0));
...
mod tests {
    ...
    fn test_translate_mov_reg_imm() {
        ...
        let op = make_opcode("mov", vec![make_reg_oper("rax", 8), make_imm_oper(42, 8)]);
    ...
    fn test_translate_mov_reg_reg() {
        ...
        let op = make_opcode(
            "mov",
            vec![make_reg_oper("rax", 8), make_reg_oper("rbx", 8)],
        );
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
        assert_eq!(effects.len(), 1);
    ...
    fn test_translate_add() {
        let mut xlate = SymbolikTranslator::new_64();
        let op = make_opcode("add", vec![make_reg_oper("rax", 8), make_imm_oper(5, 8)]);
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
        assert_eq!(effects.len(), 1);
        assert!(effects[0].sets_variable("rax"));
    ...
    fn test_translate_sub() {
        let mut xlate = SymbolikTranslator::new_64();
        let op = make_opcode("sub", vec![make_reg_oper("rsp", 8), make_imm_oper(0x28, 8)]);
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
        assert_eq!(effects.len(), 1);
    ...
    fn test_translate_xor_self() {
        ...
        let op = make_opcode(
            "xor",
            vec![make_reg_oper("eax", 4), make_reg_oper("eax", 4)],
        );
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
        assert_eq!(effects.len(), 1);
    ...
    fn test_translate_push() {
        let mut xlate = SymbolikTranslator::new_64();
        let op = make_opcode("push", vec![make_reg_oper("rbp", 8)]);
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
    ...
    fn test_translate_call() {
        let mut xlate = SymbolikTranslator::new_64();
        let op = make_opcode("call", vec![make_imm_oper(0x402000, 8)]);
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
    ...
    fn test_translate_cmp_jz() {
        ...
        // CMP rax, 0
        let cmp_op = make_opcode("cmp", vec![make_reg_oper("rax", 8), make_imm_oper(0, 8)]);
    ...
    fn test_effect_log_accumulation() {
        ...
        xlate.translate_opcode(&make_opcode(
            "mov",
            vec![make_reg_oper("rax", 8), make_imm_oper(1, 8)],
        ));
        xlate.translate_opcode(&make_opcode(
            "mov",
            vec![make_reg_oper("rbx", 8), make_imm_oper(2, 8)],
        ));
        xlate.translate_opcode(&make_opcode(
            "add",
            vec![make_reg_oper("rax", 8), make_reg_oper("rbx", 8)],
    ...
    fn test_32bit_translator() {
        ...
        let op = make_opcode("push", vec![make_reg_oper("ebp", 4)]);
        xlate.translate_opcode(&op);
        let effects = xlate.get_effects();
