    ...
    pub fn get_symbolik_outputs(&self, graph: &SymbolikGraph) -> Vec<SymbolikOutput> {
        ...
        path_results
            ...
            .map(|pr| {
                ...
                let calls = pr.effects.iter().filter(|e| e.is_call()).cloned().collect();
...
mod tests {
    ...
    fn make_opcode_at(
        va: u64,
        mnem: &str,
        opers: Vec<Box<dyn crate::envi::operand::Operand>>,
    ) -> Opcode {
        Opcode {
            va,
            opcode: 0,
    ...
    fn test_single_block_function() {
        ...
        let opcodes = vec![
            make_opcode_at(
                0x1000,
                "mov",
                vec![make_reg_oper("rax", 8), make_imm_oper(42, 8)],
            ),
            make_opcode_at(0x1003, "ret", vec![]),
        ];

        let graph = ctx.build_symbolik_graph(0x1000, vec![(0x1000, opcodes, vec![])]);
    ...
    fn test_linear_two_blocks() {
        ...
        let graph = ctx.build_symbolik_graph(
            0x1000,
            vec![
                (
                    0x1000,
                ...
                (
                    ...
                    vec![
                        make_opcode_at(
                            0x1010,
                            "add",
                            vec![make_reg_oper("rax", 8), make_imm_oper(2, 8)],
                        ),
            ...
            vec![
                (
                    ...
                    vec![make_opcode_at(
                        0x1000,
                        "cmp",
                        vec![make_reg_oper("rdi", 8), make_imm_oper(0, 8)],
                    )],
                    vec![0x1010, 0x1020],
                ),
                (
                ...
                (
                    ...
                    vec![make_opcode_at(
                        0x1020,
                        "mov",
                        vec![make_reg_oper("rax", 8), make_imm_oper(0, 8)],
                    )],
                    vec![0x1030],
                ),
    ...
    fn test_loop_bounded() {
        ...
        let graph = ctx.build_symbolik_graph(
            ...
            vec![
                (
                    0x1000,
                    vec![make_opcode_at(0x1000, "nop", vec![])],
                    vec![0x1010],
                ),
                (
                    0x1010,
                    vec![make_opcode_at(0x1010, "nop", vec![])],
                    vec![0x1000, 0x1020],
                ),
    ...
    fn test_get_symbolik_outputs() {
        ...
        assert_eq!(
            outputs[0].return_value.as_ref().unwrap().as_const(),
            Some(42)
        );
    }
    ...
    fn test_pre_effects() {
        let mut ctx = SymbolikAnalysisContext::new_64();
        ctx.set_pre_effects(vec![SymbolicEffect::SetVariable {
            va: 0,
            name: "rdi".to_string(),
        ...
        let graph = ctx.build_symbolik_graph(
            ...
            vec![(
                ...
                vec![make_opcode_at(
                    0x1000,
                    "mov",
                    vec![make_reg_oper("rax", 8), make_reg_oper("rdi", 8)],
                )],
                vec![],
            )],
        );
        ...
        let graph = ctx.build_symbolik_graph(
            ...
            vec![(
                ...
                vec![make_opcode_at(
                    0x1000,
                    "mov",
                    vec![make_reg_oper("eax", 4), make_imm_oper(42, 4)],
                )],
                vec![],
            )],
        );
