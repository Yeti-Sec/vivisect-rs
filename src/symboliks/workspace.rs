...
pub fn resolve_indirect_jump(
    ...
    for pr in &path_results {
        ...
        let jump_block = graph.blocks.values().find(|b| {
            b.opcodes.iter().any(|op| op.va == jump_va)
        });

        if jump_block.is_none() {
            continue;
        }

        // Find the jump instruction and get the target operand's symbolic value
        let jump_block = jump_block.unwrap();
...
mod tests {
    ...
    fn test_enumerate_jump_table_empty() {
        ...
        let targets = enumerate_jump_table(
            &VivWorkspace::new(),
            0x0,
            10,
        );
