...
pub fn reduce(value: &SymbolicValue) -> SymbolicValue {
    match value {
        SymbolicValue::Oper { op, lhs, rhs } => {
            ...
            if matches!(op, Op::Sub | Op::Xor) && lhs_reduced == rhs_reduced {
                return SymbolicValue::constant(0, lhs_reduced.width());
            }

            // Idempotent: x & x = x, x | x = x
            if matches!(op, Op::And | Op::Or) && lhs_reduced == rhs_reduced {
                return lhs_reduced;
            }
        ...
        SymbolicValue::Call {
            ...
            let reduced_args: Vec<SymbolicValue> = args.iter().map(reduce).collect();
            SymbolicValue::Call {
                target: target.clone(),
                args: reduced_args,
                width: *width,
...
mod tests {
    ...
    fn test_identity_add_zero() {
        let expr = SymbolicValue::add(SymbolicValue::var("rax", 8), SymbolicValue::constant(0, 8));
        let result = reduce(&expr);
    ...
    fn test_nested_reduction() {
        ...
        let expr = SymbolicValue::add(
            SymbolicValue::add(SymbolicValue::var("rax", 8), SymbolicValue::constant(0, 8)),
            SymbolicValue::add(SymbolicValue::constant(3, 8), SymbolicValue::constant(4, 8)),
        );
        let result = reduce(&expr);
        ...
        match &result {
            SymbolicValue::Oper {
                ...
                assert!(
                    matches!(lhs.as_ref(), SymbolicValue::Var { ref name, .. } if name == "rax")
                );
    ...
    fn test_associative_add_constants() {
        // (rax + 3) + 4 → rax + 7
        let expr = SymbolicValue::add(
            SymbolicValue::add(SymbolicValue::var("rax", 8), SymbolicValue::constant(3, 8)),
