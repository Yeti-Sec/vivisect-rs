...
impl SymbolicValue {
    ...
    pub fn solve(&self) -> u64 {
        match self {
            ...
            SymbolicValue::Arg { index, width } => {
                varsolve(&format!("arg{}", index), *width)
            }
            ...
            SymbolicValue::Constraint { op, lhs, rhs } => {
                ...
                let r = rhs.solve();
                match op {
                    ConstraintOp::Eq => if l == r { 1 } else { 0 },
                    ConstraintOp::Ne => if l != r { 1 } else { 0 },
                    ConstraintOp::Lt => if (l as i64) < (r as i64) { 1 } else { 0 },
                    ConstraintOp::Le => if (l as i64) <= (r as i64) { 1 } else { 0 },
                    ConstraintOp::Gt => if (l as i64) > (r as i64) { 1 } else { 0 },
                    ConstraintOp::Ge => if (l as i64) >= (r as i64) { 1 } else { 0 },
                    ConstraintOp::Ult => if l < r { 1 } else { 0 },
                    ConstraintOp::Ugt => if l > r { 1 } else { 0 },
                }
            }
        }
    }
    ...
    pub fn substitute(&self, var_name: &str, replacement: &SymbolicValue) -> SymbolicValue {
        match self {
            ...
            SymbolicValue::Call { target, args, width } => SymbolicValue::Call {
                target: target.clone(),
    ...
    pub fn structurally_equivalent(&self, other: &SymbolicValue) -> bool {
        match (self, other) {
            (
                SymbolicValue::Const { width: w1, .. },
                SymbolicValue::Const { width: w2, .. },
            ) => w1 == w2,
            (
                SymbolicValue::Var { name: n1, width: w1 },
                SymbolicValue::Var { name: n2, width: w2 },
            ) => n1 == n2 && w1 == w2,
            (
                SymbolicValue::Arg { index: i1, width: w1 },
                SymbolicValue::Arg { index: i2, width: w2 },
            ) => i1 == i2 && w1 == w2,
            (
                SymbolicValue::Mem { addr: a1, width: w1 },
                SymbolicValue::Mem { addr: a2, width: w2 },
            ) => w1 == w2 && a1.structurally_equivalent(a2),
            (
                SymbolicValue::Oper { op: o1, lhs: l1, rhs: r1 },
                SymbolicValue::Oper { op: o2, lhs: l2, rhs: r2 },
            ) => o1 == o2 && l1.structurally_equivalent(l2) && r1.structurally_equivalent(r2),
            (
                SymbolicValue::Call { target: t1, args: a1, .. },
                SymbolicValue::Call { target: t2, args: a2, .. },
            ) => {
                t1 == t2
                    && a1.len() == a2.len()
                    && a1.iter().zip(a2.iter()).all(|(x, y)| x.structurally_equivalent(y))
            }
            (
                SymbolicValue::Constraint { op: o1, lhs: l1, rhs: r1 },
                SymbolicValue::Constraint { op: o2, lhs: l2, rhs: r2 },
            ) => o1 == o2 && l1.structurally_equivalent(l2) && r1.structurally_equivalent(r2),
            _ => false,
        }
    }
    ...
    fn normalize_inner(
        ...
        match self {
            ...
            SymbolicValue::Call { target, args, width } => {
                SymbolicValue::Call {
                    target: target.clone(),
                    args: args.iter().map(|a| a.normalize_inner(reg_map, counter)).collect(),
                    width: *width,
                }
            }
            SymbolicValue::Constraint { op, lhs, rhs } => {
...
fn solve_op(op: Op, lhs: u64, rhs: u64) -> u64 {
    match op {
        ...
        Op::Mod => if rhs == 0 { 0 } else { lhs % rhs },
        Op::And => lhs & rhs,
        Op::Or => lhs | rhs,
        Op::Xor => lhs ^ rhs,
...
impl PartialEq for SymbolicValue {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (
                SymbolicValue::Const { value: v1, width: w1 },
                SymbolicValue::Const { value: v2, width: w2 },
            ) => v1 == v2 && w1 == w2,
            (
                SymbolicValue::Var { name: n1, width: w1 },
                SymbolicValue::Var { name: n2, width: w2 },
