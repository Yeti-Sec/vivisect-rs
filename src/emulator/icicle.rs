...
use icicle_cpu::{Config, ExceptionCode, ValueSource, VmExit};
use icicle_vm::Vm;
...
impl IcicleEmulator {
    ...
    pub fn load_binary(&mut self, addr: u64, data: &[u8], executable: bool) -> VivResult<()> {
        tracing::debug!(
            "[viv-emu] load_binary: addr={:#x}, size={:#x}",
            addr,
            data.len()
        );
    ...
    pub fn init_stack(&mut self, stack_base: u64, stack_size: usize) -> VivResult<()> {
        tracing::debug!(
            "[viv-emu] init_stack: addr={:#x}, size={:#x}",
            stack_base,
            stack_size
        );
    ...
    pub fn restore_full_snapshot(&mut self, snapshot: &icicle_vm::Snapshot) {
        tracing::trace!(
            "[viv-emu] restore_full_snapshot: pc_before={:#x}",
            self.get_pc()
        );
...
impl CpuEmulator for IcicleEmulator {
    ...
    fn write_register(&mut self, name: &str, value: u64) -> VivResult<()> {
        tracing::trace!(
            "[viv-emu] write_register: name={}, value={:#x}",
            name,
            value
        );
        let var = self
    ...
    fn write_memory(&mut self, addr: u64, data: &[u8]) -> VivResult<()> {
        tracing::trace!(
            "[viv-emu] write_memory: addr={:#x}, size={}",
            addr,
            data.len()
        );
        self.vm
            .cpu
            .mem
            .write_bytes(addr, data, icicle_mem::perm::NONE)
            .map_err(|_e| VivError::InvalidMemory { address: addr })
    }

    fn map_memory(&mut self, addr: u64, size: usize, perms: MemoryPermissions) -> VivResult<()> {
        tracing::debug!(
            "[viv-emu] map_memory: addr={:#x}, size={:#x}, perms={:?}",
            addr,
            size,
            perms
        );
        ...
        self.vm
            .cpu
            .mem
            .map_memory_len(addr, size as u64, icicle_mem::Mapping { perm, value: 0 });
        Ok(())
    }
    ...
    fn run_to(&mut self, target_pc: u64, max_steps: u64) -> VivResult<(u64, StopReason)> {
        tracing::trace!(
            "[viv-emu] run_to: target={:#x}, max_steps={}",
            target_pc,
            max_steps
        );
        ...
        tracing::trace!(
            "[viv-emu] run_to result: {:?}, pc={:#x}, steps={}",
            reason,
            self.get_pc(),
        ...
        let value = match self.ptr_size {
            4 => {
                let arr: [u8; 4] = bytes[..4]
                    .try_into()
                    .expect("read_memory returned ptr_size bytes");
...
mod tests {
    ...
    fn test_memory_operations() {
        ...
        // Map memory
        emu.map_memory(0x1000, 0x1000, MemoryPermissions::READ_WRITE)
            .unwrap();
