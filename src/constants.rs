...
impl Architecture {
    ...
    /// Parse an architecture from its canonical [`name`](Self::name).
    ///
    /// Inverse of `name()`; returns `None` for an unrecognized name so callers
    /// can decide whether to error or fall back (persistence never guesses).
    pub fn from_name(name: &str) -> Option<Architecture> {
        let a = match name {
            ...
            "thumb16" => Architecture::Thumb16,
            "thumb" => Architecture::Thumb,
            "msp430" => Architecture::Msp430,
            "h8" => Architecture::H8,
            "a64" => Architecture::A64,
            "rv32" => Architecture::RiscV32,
            "rv64" => Architecture::RiscV64,
            "ppc32-embedded" => Architecture::PpcE32,
            "ppc-embedded" => Architecture::PpcE64,
            "ppc32-server" => Architecture::PpcS32,
            "ppc-server" => Architecture::PpcS64,
            "ppc-vle" => Architecture::PpcVle,
            "ppc-desktop" => Architecture::PpcD,
            "mcs51" => Architecture::Mcs51,
            "rxv2" => Architecture::RxV2,
            "sparc" => Architecture::Sparc,
            "sparc64" => Architecture::Sparc64,
            "m68k" => Architecture::M68k,
            "superh" => Architecture::SuperH,
    ...
    /// Machine-readable capability flags for this architecture.
    ///
    /// Enum presence does **not** imply parity (review finding #16/#12): a
    /// variant may exist while only some analysis stages are actually
    /// implemented for it. Only x86/x86-64 currently have the full semantic
    /// pipeline; everything else is reported as unsupported so public APIs can
    /// reject cleanly instead of silently misdecoding as x86 (finding #8).
    pub fn capabilities(&self) -> ArchitectureCapabilities {
        match self {
            Architecture::I386 | Architecture::Amd64 => ArchitectureCapabilities {
                syntax_decode: true,
                semantic_operands: true,
                branch_semantics: true,
                dataflow: true,
                codeflow: true,
                // No emulation backend in default features (unicorn/icicle are
                // optional and external); reported honestly as unsupported.
                emulation: false,
            },
            _ => ArchitectureCapabilities::none(),
        }
    }

    /// Whether the full code-flow analysis pipeline supports this architecture.
    pub fn is_analysis_supported(&self) -> bool {
        self.capabilities().codeflow
    }
...
/// Per-architecture capability flags (roadmap Phase 12).
///
/// Each flag advertises whether a given analysis stage is actually implemented
/// for the architecture, so callers can gate on real capability rather than the
/// mere presence of an [`Architecture`] enum variant.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ArchitectureCapabilities {
    /// Instruction bytes can be decoded to mnemonic/operands syntax.
    pub syntax_decode: bool,
    /// Operands carry semantic structure (registers/immediates/effective addr).
    pub semantic_operands: bool,
    /// Branch targets (incl. indirect) are modeled.
    pub branch_semantics: bool,
    /// Data-flow / xref extraction is implemented.
    pub dataflow: bool,
    /// Code-flow (function/block discovery) is implemented.
    pub codeflow: bool,
    /// CPU emulation is available.
    pub emulation: bool,
}
...
impl ArchitectureCapabilities {
    ...
    /// Whether any capability at all is supported.
    pub fn any(&self) -> bool {
        self.syntax_decode
            || self.semantic_operands
            || self.branch_semantics
            || self.dataflow
            || self.codeflow
            || self.emulation
    }
...
mod tests {
    ...
    #[test]
    fn test_architecture_capabilities_x86_full() {
        for arch in [Architecture::I386, Architecture::Amd64] {
            let c = arch.capabilities();
            assert!(c.syntax_decode && c.semantic_operands && c.branch_semantics);
            assert!(c.dataflow && c.codeflow);
            assert!(!c.emulation, "no emulation backend in default features");
            assert!(arch.is_analysis_supported());
        }
    }

    #[test]
    fn test_architecture_capabilities_unsupported_are_none() {
        // Enum presence must not imply parity: non-x86 archs report no
        // capabilities and are not analysis-supported (finding #12/#8).
        for arch in [
            Architecture::Default,
            Architecture::ArmV7,
            Architecture::A64,
            ...
            let c = arch.capabilities();
            assert!(!c.any(), "{:?} unexpectedly reports a capability", arch);
            assert!(!arch.is_analysis_supported());
        }
    }

    #[test]
    fn test_architecture_from_name_roundtrip() {
        // from_name is the inverse of name() for every variant (persistence #1).
        let archs = [
            Architecture::Default,
            Architecture::I386,
            Architecture::Amd64,
        ...
        assert_eq!(Architecture::from_name("nonsense"), None);
    }
